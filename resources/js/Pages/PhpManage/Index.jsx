...
const PHPManager = () => {
    ...
    const [exts, setExts] = useState({}); // { version: [ext1, ext2, ...] }
    const [extInput, setExtInput] = useState({}); // { version: '' }
    ...
    const fetchVersions = async () => {
        ...
        try {
            ...
            // Fetch extensions for each version
            for (const row of res.data) {
                fetchExtensions(row.version);
            }
        } catch (e) {
    ...
    const fetchExtensions = async (version) => {
        try {
            const res = await axios.get(`/php/extensions?version=${version}`);
            setExts(prev => ({ ...prev, [version]: res.data.extensions || [] }));
        } catch (e) {
            setExts(prev => ({ ...prev, [version]: [] }));
        }
    };
    ...
    const handleInstallExt = async (version) => {
        const ext = extInput[version];
        if (!ext) return setAlert({ type: 'error', message: 'Enter extension name' });
        setLoading(true);
        try {
            const res = await axios.post('/php/extension/install', { version, extension: ext });
            setAlert({ type: res.data.status, message: res.data.message });
            fetchExtensions(version);
        } catch (e) {
            setAlert({ type: 'error', message: 'Install extension failed' });
        }
        setLoading(false);
    };

    const handleRemoveExt = async (version, ext) => {
        if (!window.confirm(`Remove extension ${ext} from PHP ${version}?`)) return;
        setLoading(true);
        try {
            const res = await axios.post('/php/extension/remove', { version, extension: ext });
            setAlert({ type: res.data.status, message: res.data.message });
            fetchExtensions(version);
        } catch (e) {
            setAlert({ type: 'error', message: 'Remove extension failed' });
        }
        setLoading(false);
    };
            ...
            <table className="min-w-full bg-white border mb-4">
                <thead>
                    <tr>
                        ...
                        <th className="py-2 px-4 border-b">Extensions</th>
                    </tr>
                </thead>
                <tbody>
                    {versions.map(row => (
                        <tr key={row.version}>
                            ...
                            <td className="py-2 px-4 border-b">
                                <div>
                                    <ul className="mb-2">
                                        {(exts[row.version] || []).map(ext => (
                                            <li key={ext} className="flex items-center justify-between">
                                                <span>{ext}</span>
                                                <button onClick={() => handleRemoveExt(row.version, ext)} className="ml-2 text-xs bg-red-400 text-white px-2 py-0.5 rounded">Remove</button>
                                            </li>
                                        ))}
                                    </ul>
                                    <div className="flex">
                                        <input type="text" value={extInput[row.version] || ''} onChange={e => setExtInput(prev => ({ ...prev, [row.version]: e.target.value }))} className="border px-2 py-1 text-xs" placeholder="Extension name" />
                                        <button onClick={() => handleInstallExt(row.version)} className="bg-blue-400 text-white px-2 py-1 rounded ml-2 text-xs">Install</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
